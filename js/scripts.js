...
function initRestabook() {
    ...
    // Slider-event
    const slidesEvent = document.querySelectorAll('.swiper-slide-event'),
        prevEvent = document.querySelector('.ec-button-prev'),
        nextEvent = document.querySelector('.ec-button-next');
    ...
    function showSlides(n) {
        if (n > slidesEvent.length) {
            slideEventIndex = 1;
        }
        if (n < 1) {
            slideEventIndex = slidesEvent.length;
        }
        slidesEvent.forEach(item => {
            item.classList.add('hide');
            item.classList.remove('show')
        });
        slidesEvent[slideEventIndex - 1].classList.remove('hide');
        slidesEvent[slideEventIndex - 1].classList.add('show');
    }
    ...
    // Slider-comments
    const slidesComments = document.querySelectorAll('.swiper-slide'),
        prevComments = document.querySelector('.tc-button-prev'),
        nextComments = document.querySelector('.tc-button-next');

    let slideCommentsIndex = 1;

    showSlidesC(slideCommentsIndex);

    function showSlidesC(n) {
        if (n > slidesComments.length) {
            slideCommentsIndex = 1;
        }
        if (n < 1) {
            slideCommentsIndex = slidesComments.length;
        }
        slidesComments.forEach(item => {
            item.classList.add('hide');
            item.classList.remove('show')
        });
        slidesComments[slideCommentsIndex - 1].classList.remove('hide');
        slidesComments[slideCommentsIndex - 1].classList.add('show');
    }

    function plusSlidesC(n) {
        showSlidesC(slideCommentsIndex += n);
    }
